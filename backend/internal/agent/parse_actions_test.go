...
func TestEveryAdvertisedActionParses(t *testing.T) {
	...
	minimal := map[protocol.ActionKind]string{
		...
		protocol.ActCallMCP:      `{"action":"call_mcp","mcp_tool_name":"get_weather","mcp_params":{"city":"Boise"}}`,
...
func TestSummariseCoversTheNewActions(t *testing.T) {
	cases := []struct {
		...
		{"call_mcp names the tool", protocol.Action{Action: protocol.ActCallMCP, MCPToolName: "get_weather"}, "call_mcp get_weather"},
