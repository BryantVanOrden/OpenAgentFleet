...
func TestParseActionRejects(t *testing.T) {
	cases := []struct {
		...
		{"click with neither coordinates nor target", `{"action":"click"}`, "needs mark, coordinates or a target"},
		{"click with blank target", `{"action":"click","target":"   "}`, "needs mark, coordinates or a target"},
		{"right_click with neither", `{"action":"right_click"}`, "needs mark, coordinates or a target"},
