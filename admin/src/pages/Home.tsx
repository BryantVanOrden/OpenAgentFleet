...
import { SummaryRow, kindChipClass } from "../components/CommsTab";
import { Button, ErrorNote, cx, relative } from "../components/ui";

/**
 * The console's front door: one chat with the whole fleet.
 *
...
export default function Home({ role }: { role: string }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [list, insts, taskList] = await Promise.all([
    ...
    <div className="flex h-full flex-col">
      ...
      <div ref={listRef} className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
        <div className="mx-auto w-full max-w-3xl space-y-3">
          ...
          {loading ? (
            <p className="py-16 text-center text-xs text-ink-400">Loading the channel…</p>
          ) : stream.length === 0 ? (
            <EmptyChannel />
          ) : (
            stream.map((item) =>
            ...
            <textarea
              ...
              placeholder={
                readOnly
                  ...
                  : "Message the fleet, or type / for commands"
              }
              value={draft}
              disabled={busy || readOnly}
              onChange={(e) => {
