...
The same device queue runs **external agents**. A Claude Code, Codex or Hermes agent is an instance with a `kind` and a `connection` (device, folder, model, autonomy); a ticket's run becomes an `agent_run` device job (`internal/external/device.go`), which the host executes with `runtimes.py` — the CLI in the agent's folder, progress posted every two seconds and a stop honoured from the console, the answer, session id, tokens, cost and the text files it produced posted back. The orchestrator records the turn like a desktop's, publishes the files to the work catalog, and finishes the task, so the ticket engine never knows the difference. OpenClaw (`openclaw.go`) and webhooks (`webhook.go`) are the same contract without a device. Work reaches all of them through **tickets** (`internal/tickets`), and the console's *Work* and *Org* pages and the app's matching screens are views of `/api/tickets` and `/api/org`, refreshed by `ticket` events. See [ORG-AND-TICKETS.md](ORG-AND-TICKETS.md).
