...
- **Where a webhook or gateway may be.** The orchestrator POSTs a run's
  brief to a webhook agent's address and shows the answer as the ticket's
  result, so an address is a request made from inside the orchestrator's
  network. Loopback and link-local addresses (cloud metadata services live
  at 169.254.169.254) are refused for everyone; an address on a private
  network (10/8, 172.16/12, 192.168/16, 100.64/10, fc00::/7) only when an
  admin sets it, which is recorded on the agent and never taken from a
  client or a template. The host is resolved and checked at every
  connection, redirects included, and no proxy dials on the orchestrator's
  behalf. `AGENTFLEET_EXTERNAL_ALLOW_LOOPBACK=1` allows loopback for a
